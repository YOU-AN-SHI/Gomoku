...
# 程式名稱 : 五子棋
# 遊玩人數 : 2人
# 使用說明 : 遊戲畫面一開始會停留在初始畫面，按任意鍵即可開始遊戲
...
while 1:
    ...
    # 將所有棋子畫出來
    for chess in played:
        pygame.draw.circle(game_screen, chess[1], chess[0], 20, 0)
    ...
    if res[0] != 0:
        for chess in res[1]:
            pygame.draw.rect(game_screen, [238, 48, 167], [chess[0] * 44 + 27 - 22, chess[1] * 44 + 27 - 22, 44, 44], 2, 1)
        # 遊戲結束時，將準備進入遊戲結束畫面
        game_over = True
        # 畫面更新
        pygame.display.update()
    ...
    if press[0] and time == 0:
        ...
        if Check(x, y, played):
            ...
            # 奇數回合(已下偶數個棋)為黑子回合
            if len(played) % 2 == 0:
                played.append([[x, y], BLACK])
            # 否則為白子回合
            else:
                played.append([[x, y], WHITE])
